...
class renderer extends plugin_renderer_base {
    ...
    protected function render_manage_data(manage_data $sessdata) {
        ...
        $o = $this->output->container($o, 'generalbox container-fluid');
    ...
    protected function render_sess_manage_table(manage_data $sessdata) {
        ...
        foreach ($sessdata->sessions as $key => $sess) {
            ...
            // Merge Date and Time.
            $datetime = $dta['date'] . html_writer::empty_tag('br') . $dta['time'];
            $table->data[$sess->id][] = $datetime;
    ...
    protected function render_take_data(take_data $takedata) {
        ...
        // Add JavaScript for cycling status button functionality
        $this->page->requires->js_amd_inline("
            require(['jquery'], function($) {
                $(document).ready(function() {
                    // Handle single status button clicks (cycling functionality)
                    $(document).on('click', '.single-status-btn', function(e) {
                        e.preventDefault();
                        var button = $(this);
                        var userid = button.data('user-id');
                        var currentStatusId = button.data('current-status-id');
                        var statuses = button.data('statuses');
                        var hiddenInput = $('.status-input-' + userid);
                        ...
                        // Get next status (cycle through all statuses)
                        var nextIndex = (currentIndex + 1) % statuses.length;
                        var nextStatus = statuses[nextIndex];
                        
                        // Update button display
                        button.text(nextStatus.acronym);
                        button.attr('title', nextStatus.description);
                        button.data('current-status-id', nextStatus.id);
                        
                        // Remove all status classes and add new one
                        button.removeClass(function(index, className) {
                            return (className.match(/(^|\s)status-\S+/g) || []).join(' ');
                        });
                        button.addClass('status-' + nextStatus.acronym.toLowerCase());
                        button.addClass('has-status');
                        
                        // Update hidden input
                        hiddenInput.val(nextStatus.id);
                        
                        // Visual feedback
                        button.addClass('status-changed');
                        setTimeout(function() {
                            button.removeClass('status-changed');
                        }, 200);
                    });
                    
                    // Handle 'Set All' button functionality
                    $(document).on('click', '.set-all-status-cycle', function(e) {
                        e.preventDefault();
                        var button = $(this);
                        var currentCycleStatus = button.data('current-cycle-status') || 0;
                        
                        // Get all status buttons
                        var statusButtons = $('.single-status-btn:visible');
                        if (statusButtons.length === 0) return;
                        
                        // Get statuses from first button (they all have the same statuses)
                        var statuses = statusButtons.first().data('statuses');
                        if (!statuses || statuses.length === 0) return;
                        
                        var targetStatus = statuses[currentCycleStatus % statuses.length];
                        
                        // Set all buttons to this status
                        statusButtons.each(function() {
                            ...
                            // Update button
                            btn.text(targetStatus.acronym);
                            btn.attr('title', targetStatus.description);
                            btn.data('current-status-id', targetStatus.id);
                            
                            // Remove all status classes and add new one
                            btn.removeClass(function(index, className) {
                                return (className.match(/(^|\s)status-\S+/g) || []).join(' ');
                            });
                            btn.addClass('status-' + targetStatus.acronym.toLowerCase());
                            btn.addClass('has-status');
                            
                            // Update hidden input
                            hiddenInput.val(targetStatus.id);
                        });
                        
                        // Update button text and cycle to next status
                        button.text('Set All: ' + targetStatus.acronym);
                        button.data('current-cycle-status', currentCycleStatus + 1);
                        
                        // Visual feedback
                        statusButtons.addClass('status-changed');
                        setTimeout(function() {
                            statusButtons.removeClass('status-changed');
                        }, 300);
                    });
                    
                    // Initialize 'Set All' button
                    $('.set-all-status-cycle').each(function() {
                        var button = $(this);
                        var firstStatusBtn = $('.single-status-btn:first');
                        if (firstStatusBtn.length > 0) {
                            var statuses = firstStatusBtn.data('statuses');
                            if (statuses && statuses.length > 0) {
                                button.text('Set All: ' + statuses[0].acronym);
                            }
                        }
                    });
                });
        ...
        // Add CSS styling for single status button system  
        $this->page->requires->js_amd_inline("
            require(['jquery'], function($) {
                $(document).ready(function() {
                    ...
                    var cssStyles = `
                        <style>
                            /* Single Status Button Attendance System */
                            .attendance-single-status {
                                width: 100%;
                            ...
                            .single-status-btn:hover {
                                transform: translateY(-2px);
                                box-shadow: 0 4px 8px rgba(0,0,0,0.2);
                            }
                            
                            /* Status Color Coding */
                            .single-status-btn.status-p,
                            .single-status-btn.status-present {
                                background-color: #28a745;
                                color: white;
                                border-color: #28a745;
                            }
                            
                            .single-status-btn.status-a,
                            .single-status-btn.status-absent {
                                background-color: #dc3545;
                                color: white;
                                border-color: #dc3545;
                            }
                            
                            .single-status-btn.status-l,
                            .single-status-btn.status-late {
                                background-color: #ffc107;
                                color: #212529;
                                border-color: #ffc107;
                            }
                            
                            .single-status-btn.status-e,
                            .single-status-btn.status-excused {
                                background-color: #17a2b8;
                                color: white;
                                border-color: #17a2b8;
                            }
                            
                            .single-status-btn.status-unmarked {
                                background-color: #f8f9fa;
                                color: #6c757d;
                                border-color: #6c757d;
                                border-style: dashed;
                            ...
                            .single-status-btn.status-changed {
    ...
    protected function render_attendance_take_controls(take_data $takedata) {
        ...
        $content = html_writer::div($this->construct_take_session_info($takedata), 'session-info mb-3');
        $content .= html_writer::div($this->construct_take_controls($takedata), 'take-controls');

        $return .= $this->output->container($content, 'generalbox takecontrols text-left d-flex flex-column w-100');
        return $return;
    }
    ...
    private function construct_take_session_info(take_data $takedata) {
        ...
        $sessinfo = $date . html_writer::empty_tag('br') . $time;
    ...
    private function construct_take_controls(take_data $takedata) {
        ...
        // View mode and pagination dropdowns removed as per user request.
        ...
        return html_writer::div($controls, 'text-left');
    }
    ...
    protected function render_attendance_take_list(take_data $takedata) {
        ...
        if (!empty($CFG->showuseridentity) && has_capability('moodle/site:viewuseridentity', $takedata->att->context)) {
            ...
            // Hide email field by default (can be shown via preferences)
            $extrasearchfields = array_filter($extrasearchfields, function($field) {
                return $field !== 'email';
            });
        }
        ...
        // Add single status column for all devices
        $table->head[] = html_writer::tag('span', get_string('status'), ['class' => 'single-status-column']);
        $table->align[] = 'center';
        $table->size[] = '120px';
        ...
        $table->align[] = 'center';
        $table->size[] = '20px';
        $table->attributes['class'] = 'generaltable takelist attendance-single-status';
        ...
        // Add single "Set All" status button that cycles through all statuses
        $setAllButton = html_writer::tag('button', get_string('setallstatus', 'attendance'), [
            'type' => 'button',
            'id' => 'setAllStatusBtn',
            'class' => 'btn btn-sm btn-secondary set-all-status-cycle',
            'title' => get_string('clicktocyclestatuses', 'attendance')
        ]);
        $row->cells[] = $setAllButton;
        ...
        foreach ($takedata->users as $user) {
            $i++;
            $row = new html_table_row();
            $row->attributes['class'] = 'mobile-student-row';
            $row->attributes['data-userid'] = $user->id;
            $row->attributes['data-user-index'] = $i;
            
            // Create clickable name for mobile
            $fullname = html_writer::link($takedata->url_view(['studentid' => $user->id]), fullname($user), [
                'class' => 'mobile-student-name',
                'data-user-id' => $user->id,
                'data-user-index' => $i,
                'data-student-id' => $user->idnumber ?? $user->id
            ]);
            ...
            // Add single status cell that works for both mobile and desktop
            $ucdata = $this->construct_take_user_controls($takedata, $user);
            if (array_key_exists('colspan', $ucdata)) {
                $cell = new html_table_cell($ucdata['text']);
                $cell->colspan = $ucdata['colspan'];
                $cell->attributes['class'] = 'single-status-cell';
                $row->cells[] = $cell;
                // Add empty remarks cell to maintain column structure
                $row->cells[] = '';
            } else {
                // Add the single status button cell
                $statusCell = new html_table_cell($ucdata['statusbutton']);
                $statusCell->attributes['class'] = 'single-status-cell';
                $row->cells[] = $statusCell;
                
                // Add remarks cell
                $remarksCell = new html_table_cell($ucdata['remarks']);
                $remarksCell->attributes['class'] = 'remarks-cell';
    ...
    protected function render_attendance_take_grid(take_data $takedata) {
        ...
        foreach ($takedata->statuses as $st) {
            $buttonattribs = [
                'type' => 'button',
                'id' => 'checkstatus'.$st->id,
                'class' => 'btn btn-sm btn-secondary select-all-status',
                'title' => get_string('setallstatusesto', 'attendance', $st->description),
                'data-status-id' => $st->id
            ];
            $head[] = html_writer::tag('button', $st->acronym, $buttonattribs);
            // JS to select all buttons of this status and prevent default behaviour.
            $this->page->requires->js_amd_inline("
                 require(['jquery', 'core_user/repository'], function($, UserRepository) {
                     $('#checkstatus".$st->id."').click(function(e) {
                         ...
                         if ($('select[name=\"setallstatus-select\"] option:selected').val() == 'unselected') {
                             $('.attendance-status-btn.st' + statusId + ':not(.selected)').click();
                             UserRepository.setUserPreference('mod_attendance_statusdropdown','unselected');
                         } else {
                             $('.attendance-status-btn.st' + statusId).click();
    ...
    /**
     * Create status button for attendance taking
     *
     * @param stdClass $status The status object
     * @param string $name The input name attribute
     * @param string $userid The user ID
     * @param boolean $selected Whether this status is selected
     * @param boolean $isgrid Whether this is for grid view
     * @return string HTML for the status button
     */
    private function create_status_button($status, $name, $userid, $selected = false, $isgrid = false) {
        $classes = 'attendance-status-btn st' . $status->id;
        if ($selected) {
            $classes .= ' selected';
        }
        ...
        $buttontext = $isgrid ? $status->acronym : $status->acronym;
        $button = html_writer::tag('button', $buttontext, $attributes);
        
        // Only create one hidden input per user (not per status)
        // The JavaScript will handle setting the correct value
        static $hiddenInputsCreated = [];
        $hidden = '';
        if (!isset($hiddenInputsCreated[$name])) {
            $hiddenattributes = [
                'type' => 'hidden',
                'name' => $name,
                'value' => $selected ? $status->id : '',
                'class' => 'status-input-' . $userid
            ];
            $hidden = html_writer::empty_tag('input', $hiddenattributes);
            $hiddenInputsCreated[$name] = true;
        }
    ...
    private function construct_take_user_controls(take_data $takedata, $user) {
        ...
        if ($user->enrolmentend && $user->enrolmentend < $takedata->sessioninfo->sessdate) {
            ...
            // Create single cycling status button
            $currentStatus = null;
            $currentStatusId = '';
            if (array_key_exists($user->id, $takedata->sessionlog)) {
                $currentStatusId = $takedata->sessionlog[$user->id]->statusid;
                foreach ($takedata->statuses as $st) {
                    if ($st->id == $currentStatusId) {
                        $currentStatus = $st;
                        break;
                    }
                }
            ...
            // Determine button display and styling
            $buttonText = $currentStatus ? $currentStatus->acronym : '?';
            $buttonClass = 'single-status-btn';
            $buttonTitle = $currentStatus ? $currentStatus->description : get_string('clicktosetattendance', 'attendance');
            
            if ($currentStatus) {
                $buttonClass .= ' status-' . strtolower($currentStatus->acronym);
                $buttonClass .= ' has-status';
            } else {
                $buttonClass .= ' status-unmarked';
            }
            
            // Sort statuses in desired order: P (Present), A (Absent), L (Late), E (Excused)
            $sortedStatuses = [];
            $statusOrder = ['P', 'A', 'L', 'E'];
            foreach ($statusOrder as $acronym) {
                foreach ($takedata->statuses as $status) {
                    if (strtoupper($status->acronym) === $acronym) {
            ...
            foreach ($takedata->statuses as $status) {
                $found = false;
                foreach ($sortedStatuses as $sorted) {
                    if ($sorted->id === $status->id) {
                        $found = true;
                        break;
                    }
                }
            ...
            $statusButton = html_writer::tag('button', $buttonText, [
                'type' => 'button',
                'class' => $buttonClass,
                'data-user-id' => $user->id,
                'data-current-status-id' => $currentStatusId,
                'data-statuses' => json_encode(array_values($sortedStatuses)),
                'title' => $buttonTitle,
                'aria-label' => get_string('attendancefor', 'attendance', fullname($user))
            ]);
            ...
            $hiddenInput = html_writer::empty_tag('input', [
                ...
                'class' => 'status-input-' . $user->id
            ]);
            
            $celldata['statusbutton'] = $statusButton . $hiddenInput;
            ...
            if (array_key_exists($user->id, $takedata->sessionlog)) {
                $remarksParams['value'] = $takedata->sessionlog[$user->id]->remarks;
            }
            $celldata['remarks'] = html_writer::empty_tag('input', $remarksParams);
    ...
    private function construct_take_session_controls(take_data $takedata, $user) {
        ...
        if ($user->enrolmentend && $user->enrolmentend < $takedata->sessioninfo->sessdate) {
            ...
            $celldata['text'] = [];
            $isgrid = ($takedata->pageparams->viewmode == mod_attendance_take_page_params::SORTED_GRID);
            
            foreach ($takedata->statuses as $st) {
                $selected = array_key_exists($user->id, $takedata->sessionlog) && 
                           $st->id == $takedata->sessionlog[$user->id]->statusid;
                           
                $button = $this->create_status_button($st, 'user'.$user->id.'sess'.$takedata->sessioninfo->id, $user->id, $selected, $isgrid);
                $celldata['text'][] = $button;
            }
            $params = [
                    'type'  => 'text',
    ...
    private function construct_user_data(user_data $userdata) {
        ...
        if ($userdata->pageparams->mode == mod_attendance_view_page_params::MODE_THIS_COURSE) {
            ...
            $o .= attendance_construct_user_data_stat($userdata->summary->get_all_sessions_summary_for($userdata->user->id),
